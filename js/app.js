...
// Ensure initialization when sections are already in the DOM (static build)
(function ensureAppInitOnce() {
  if (window.__joyAppInitHookAdded) return;
  window.__joyAppInitHookAdded = true;

  const start = () => {
    if (window.__joyAppInitialized) return;
    window.__joyAppInitialized = true;
...
// ============ FLAVOR MODAL ============
function initFlavorModal() {
  const flavorButtons = document.querySelectorAll(".flavor-btn");
  const closeButtons = document.querySelectorAll(".flavor-modal-close");
  const allModals = document.querySelectorAll(".flavor-modal-popup");

  if (flavorButtons.length === 0) return;
  ...
  function toggleModal(flavorId) {
    const card = document.querySelector(`[data-flavor="${flavorId}"]`);
    if (!card) return;

    const modal = card.querySelector(".flavor-modal-popup");
    if (!modal) return;
